...
if grep -rq '//hostname "Xonotic $g_xonoticversion Server"' ${SERVER_DIR}/data/server.cfg; then
	sed -i '/\/\/hostname "AAA Edgegap Server version $g_xonoticversion"/c\hostname "Xonotic Docker"	// this name will appear on the server list (the $g_xonoticversion gets replaced with the current version)' ${SERVER_DIR}/data/server.cfg
fi
echo "---Server ready---"
